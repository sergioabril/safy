...
open class QRCode: NSObject, AVCaptureMetadataOutputObjectsDelegate {
    ...
    public override init() {
        self.lineWidth = 3
        self.strokeColor = globalcolor
        self.maxDetectedCount = 20
        self.autoRemoveSubLayers = true
    ...
    class open func generateImage(_ stringValue: String, showSafyFrame: Bool = false, avatarImage: UIImage?, avatarScale: CGFloat = 0.25) -> UIImage? {
        return generateImage(stringValue, showSafyFrame: showSafyFrame, avatarImage: avatarImage, avatarScale: avatarScale, color: CIColor(color: UIColor.black), backColor: CIColor(color: UIColor.white))
    }
    ...
    ///
    ///  - returns: the generated image
    class open func generateImage(_ stringValue: String, showSafyFrame: Bool, avatarImage: UIImage?, avatarScale: CGFloat = 0.20, color: CIColor, backColor: CIColor) -> UIImage? {
        ...
        var image = UIImage(ciImage: transformedImage)
        
        //If has to be an avatar
        if avatarImage != nil {
            image = insertAvatarImage(image, avatarImage: avatarImage!, scale: avatarScale)
        }
        
        //Inside safy frame
        if(showSafyFrame == true){
            return insertIntoSafyFrame(image)
        }
    ...
    class func insertIntoSafyFrame(_ codeImage: UIImage) -> UIImage {
        
        let rect = CGRect(x: 0, y: 0, width: codeImage.size.width*2, height: codeImage.size.height*2)
        UIGraphicsBeginImageContext(rect.size)
        ...
        //Draw QRcode in the midle
        let codeposition = CGRect(x: codeImage.size.width/2, y: codeImage.size.height/2, width: codeImage.size.width, height: codeImage.size.height)
        codeImage.draw(in: codeposition)
        
        //Draw avatar at the bottom
        let safylogo = UIImage(named: "fileprotected")!
        let logow = codeImage.size.height/4;
        let logoh = logow;
        safylogo.draw(in: CGRect(x: rect.size.width/2-logow/2, y: rect.size.height - codeImage.size.height/4 - logoh/2, width: logow, height: logoh))
